...
const handleSubmit = async () => {
  ...
  try {
    const response = await axios.put(`http://localhost:7000/jobs/${jobId}`, updatedJob);
...
onMounted(async () => {
  try {
    const response = await axios.get(`http://localhost:7000/jobs/${jobId}`);
      ...
      <div class="bg-white px-6 py-8 mb-4 shadow-md rounded-md border m-4 md:m-0">
        <form @submit.prevent="handleSubmit">
          ...
          <div class="mb-4">
            ...
            <input v-model="form.title" type="text" id="title" name="title" class="border rounded w-full py-2 px-3 mb-2"
              placeholder="eg. Senior Vue Developer" required />
          </div>
